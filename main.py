...
@ti.func
def bh_smoothstep(edge1, edge2, v):
    assert(edge1 != edge2)
    t = (v-edge1) / float(edge2-edge1)
    t = hsf.clamp(t, 0.0, 1.0)
...
def render(time:ti.f32):
    ...
    for i,j in pixels:
        ...
        len = uv.norm()
        # v1 *= hsf.smoothstep(0.8, 0.0, len)
        # v2 *= hsf.smoothstep(0.6, 0.0, len)
        ...
        r = (uv * ti.Vector([0.5, 0.7])).norm()
        r_ = (uv * ti.Vector([0.8, 0.5])).norm()
        if r <2.4e-2:
            color = ti.Vector([0.0, 0.0, 0.0]) + bh_smoothstep(0, 2.4e-2, r_)
            #color = ti.Vector([1.0, 1.0, 1.0]) * bh_smoothstep(0, 2.4e-2, r_)
        else:
